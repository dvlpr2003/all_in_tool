...
import {AddPptFile} from './Addimg/Add_ppt_file'
import DashboardEdit from "./DashboardEdit"
import { DashboardNav } from "./Nav/DashboardNav"
import "./style/index.css"
import { useState } from "react"
import { useDispatch, useSelector } from "react-redux"
import {setDonwloadID} from "./ppttopdfSlicer"
...
export default function PPTtopdf(){
    ...
    const PPTItems = useSelector((state)=>state.ppt.PPT_Items)
    const globDispatch = useDispatch()
    const navigate = useNavigate()

    async function handleConvert(){
            formdata.append("items",JSON.stringify(PPTItems))
            if (PPTItems.length != 0 ){
                    
                try{
                    setLoading(true)
                    const response = await axios.post("http://127.0.0.1:8000/convert_to_pdf/convert-ppt-to-pdf/",formdata,{
                        headers: {  
        ...
        <section className="  h-screen  w-full  flex justify-center  ">
            <div className=" w-full max-w-screen-2xl h-auto bg-inherit flex min-[55px]:flex-col lg:flex-row relative z-0 mt-0">
                ...
                <div className=" w-full h-full padd flex relative min-[55px]:order-1 lg:order-2 bg-slate-50 flex-wrap overflow-scroll">
                    <AddPptFile setLoading={setLoading}/>
